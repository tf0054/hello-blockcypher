...
var Agent2 = function (_Component) {
  _inherits(Agent2, _Component);
  ...
  function Agent2() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Agent2.__proto__ || Object.getPrototypeOf(Agent2)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
      starting: false
        ...
        (0, _utils.unlockAccount)(this.props.web3.personal, this.props.lockedTime, this.props.account, this.props.password, function (result, unlockTime) {
          if (result) {
            (function () {
              ...
              var gas = instance.addOrganization.estimateGas(_this2.props.profile.name, _this2.props.profile.identity, { from: _this2.props.account });
              console.log('addOrganization {gas: %d}', gas);

              var transactionHash = instance.addOrganization(_this2.props.profile.name, _this2.props.profile.identity, { from: _this2.props.account, gas: gas });
              console.log('addOrganization {transactionHash: %s}', transactionHash);

              (0, _utils.watchTransaction)(eth, transactionHash, function (err, receipt) {
                if (!err) {
                  console.log(receipt);

                  var agent = instance.getOrganizationAgent({ from: _this2.props.account });
                  console.log('addOrganization {agent: %s}', agent);
          ...
          _react2.default.createElement(
            ...
            _react2.default.createElement(
              ...
              { bsStyle: 'info', onClick: this.startAgentClick.bind(this),
                disabled: this.props.account.length == 0 || this.props.password.length == 0 || this.props.balance.toNumber() == 0 || !this.props.profile || this.props.abi.length == 0 || this.props.systemAgent.length == 0 || this.props.agent.length > 0 || this.state.starting
              },
